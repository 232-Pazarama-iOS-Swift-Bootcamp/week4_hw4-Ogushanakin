...
final class MainTabController: UITabBarController {
    ...
    private func configureViewControllers(withUser user: User) {
        ...
        let search = templateNavigationController(unselectedImage: UIImage(named: "search_unselected")!, selectedImage: UIImage(named: "search_selected")!, rootViewController: HomeViewController())
    ...
    func didFinishPickingMedia(_ picker: YPImagePicker) {
        picker.didFinishPicking { items, _ in
            picker.dismiss(animated: false) {
                guard let selectedImsge = items.singlePhoto?.image else { return }
                
                let controller = UploadPostController()
                controller.selectedImage = selectedImsge
                controller.delegate = self
...
extension MainTabController: UploadPostControllerDelegate {
    func controllerDidFinishUploadingPost(_ controller: UploadPostController) {
        ...
        /* guard let feedNav = self.viewControllers?.first as? UINavigationController else { return }
